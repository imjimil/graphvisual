export const welshPowellColoring = (vertices, edges, currentIndex) => {
  ...
  sortedNodes.forEach(node => {
    ...
    links.forEach(link => {
      if (link.source.id === node.id && nodeColors[link.target.id]) {
        adjacentColors.add(nodeColors[link.target.id]);
      }
      if (link.target.id === node.id && nodeColors[link.source.id]) {
        adjacentColors.add(nodeColors[link.source.id]);
      }
    });

    // Find first color not used by adjacent nodes
    for (let i = 0; i < colorList.length; i++) {
      if (!adjacentColors.has(colorList[i])) {
